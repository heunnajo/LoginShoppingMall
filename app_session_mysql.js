...
var session = require('express-session');//express-session이 mysql에 접속할 수 있어야한다.
var MySQLStore = require('express-mysql-session')(session);//mysql DB에 session이라는 테이블을 생성한다.
...
app.use(session({
    ...
    store:new MySQLStore({//express가 mysql에 접속하기 위한 정보
        host:'localhost',
        port:3306,
        user:'abigail',
        password:'forever0!',
        database:'student'
    })//session 데이터를 mysql에 저장
  }));
//routing
